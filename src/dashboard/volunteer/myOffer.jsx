...
import './myOffer.css'; // Import the CSS file for styling
  ...
  useEffect(() => {
    ...
    async function fetchEvents() {
      try {
        ...
        const appointedBy = latest_user_data['data']['appointedBy']
        const request_data = {
            ids: appointedBy
        }
        const response = await fetch(`${BASE_URL}auth/getByIds`,{
            method: "POST",
            headers: {
            "Content-Type": "application/json"
            },
          ...
          .map(event => (
            <li key={event.id} className="event-item">
              <div className="event-details">
                ...
                <p className="event-info">Date: {event.updatedAt}</p>
